...
import { runUserOnboarding } from "../lib/jobs";
...
export function UserOnboarding({
  ...
  const handleSubmit = async () => {
    ...
    setStatusMessage(translate(language, "userOnboarding.status.running"));

    try {
      await runUserOnboarding({
        workspaceId,
        values: {
      ...
      setStatusMessage(translate(language, "userOnboarding.status.done"));
      onSuccess();
    } catch (error) {
      ...
      setErrorMessage(error instanceof Error ? error.message : "User onboarding failed.");
    } finally {
  ...
  return (
    <section aria-label="User onboarding chat" style={shellStyle}>
      <aside style={sidebarStyle}>
        <p style={eyebrowStyle}>{workspaceName}</p>
        <h1 style={sidebarTitleStyle}>{translate(language, "userOnboarding.title")}</h1>
        <p style={sidebarBodyStyle}>{translate(language, "userOnboarding.message")}</p>
      </aside>
      <main style={chatStyle}>
        <div style={messagesStyle}>
          <article style={assistantMessageStyle}>
            <p style={messageMetaStyle}>{translate(language, "chat.assistant")}</p>
            <p style={messageTextStyle}>{translate(language, "userOnboarding.message")}</p>
          </article>
        ...
        <div style={composerStyle}>
          <button type="button" onClick={handleSubmit} disabled={!isFormValid || isSubmitting} style={buttonStyle}>
            {isSubmitting
              ? translate(language, "userOnboarding.submitting")
              : translate(language, "userOnboarding.submit")}
          </button>
        </div>
      </main>
    </section>
